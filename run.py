...
results = [pool.apply_async(preprocess_pngs, args(x,) for x in tqdm(os.listdir(os.path.join('tmp', 'images')))]
[r.get() for r in results]
results = [pool.apply_async(write_proposals, args=(x,) for x in os.listdir(img_d))]
[r.get() for r in results]
...
if not os.path.exists('xml'):
    os.makedirs('xml')
classes = ICDAR_convert.keys()
for idx, batch in enumerate(tqdm(loader, desc="batches", leave=False)):
    ex, _, _, proposals, idn = batch
    ex = ex.to(device)
    rois, cls_preds, cls_scores, bbox_deltas = model(ex, device, proposals=proposals)
    pred_box = rois + bbox_deltas
    zipped = zip(cls_preds, pred_box)
    model2xml(idn, 'xml', [1920, 1920], zipped, classes, cls_scores)
...
results = [pool.apply(convert_to_html, args=(x,) for x in os.listdir('xml'))]
[r.get() for r in results]
#shutil.rmtree('xml')
#shutil.rmtree('tmp')
